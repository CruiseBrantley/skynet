...
describe('YouTubeMetadata', () => {
  describe('extractVideoId', () => {
    test('extracts from standard URL', () => {
      expect(
        youtube.extractVideoId('https://www.youtube.com/watch?v=dQw4w9WgXcQ')
      ).toBe('dQw4w9WgXcQ')
    })

    test('extracts from short URL', () => {
      expect(youtube.extractVideoId('https://youtu.be/dQw4w9WgXcQ')).toBe(
        'dQw4w9WgXcQ'
      )
    })
    ...
    test('extracts from shorts URL', () => {
      expect(
        youtube.extractVideoId('https://www.youtube.com/shorts/dQw4w9WgXcQ')
      ).toBe('dQw4w9WgXcQ')
    })
  ...
  describe('isYouTubeURL', () => {
    test('identifies valid domains', () => {
      ...
      expect(youtube.isYouTubeURL('https://music.youtube.com/watch?v=x')).toBe(
        true
      )
  ...
  describe('_getBestThumbnail', () => {
    test('prioritizes maxres from API', () => {
      ...
      expect(youtube._getBestThumbnail(thumbs)).toBe(
    ...
    test('passes through hqdefault URLs with query parameters unchanged', () => {
      ...
      const hq = {
        high: { url: 'https://i.ytimg.com/vi/abc/hqdefault.jpg?sqp=123' }
      }
      expect(youtube._getBestThumbnail(hq)).toBe(
        'https://i.ytimg.com/vi/abc/hqdefault.jpg?sqp=123'
      )
    })
  })
  ...
  describe('getRecommendation fallback and filtering', () => {
    beforeEach(() => {
      jest.spyOn(youtube, 'search').mockImplementation(() =>
        Promise.resolve([
          {
    ...
    test('filters out items in history', async () => {
      ...
      const lastTrack = {
        ...
        url: 'https://youtube.com/watch?v=basebasebas'
      }
      const rec = await youtube.getRecommendation([lastTrack], history)

      expect(rec[0].url).toBe('https://youtube.com/watch?v=22222222222') // 1 is filtered by history
    })
    ...
    test('filters out highly similar titles safely', async () => {
      ...
      expect(rec[0].url).toBe('https://youtube.com/watch?v=33333333333')
    })
  })
})
