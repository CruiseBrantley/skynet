...
describe('AgentLoop - Proactive Presence', () => {
    ...
    beforeEach(() => {
        ...
        mockMessages.first = jest.fn().mockImplementation(() => [...mockMessages.values()][0]);
        mockMessages.map = jest.fn().mockImplementation((fn) => [...mockMessages.values()].map(fn));
    ...
    test('handles both reactions and interjections in one pass', async () => {
        mockMessages.set('msg-old-1', { id: 'msg-old-1', author: { username: 'u' }, content: 'hi', createdAt: new Date(Date.now()) });
        mockMessages.set('msg-old-2', { id: 'msg-old-2', author: { username: 'u' }, content: 'hello', createdAt: new Date(Date.now()) });
        ...
        const msg = { 
            ...
            react: jest.fn(),
            reactions: { cache: { find: jest.fn().mockReturnValue(null) } }
        };
        ...
        expect(mockChannel.send).toHaveBeenCalledWith(expect.stringContaining('incredible'));
        expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Interjected'));
        expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Proactively reacted'));
    });
