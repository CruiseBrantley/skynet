...
module.exports = {
  name: 'unpin_message',
  description: 'Unpins a Discord message from the channel pins list by its Snowflake message ID.',
  schema: {
    message_id: {
      type: 'string',
      description: 'The Snowflake ID of the message to unpin.'
    },
    channel_id: {
      type: 'string',
  ...
  execute: async (bot, channel, params = {}, context = {}) => {
    ...
    try {
      ...
      const message = await targetChannel.messages.fetch(messageId).catch((err) => {
        logger.warn(`unpin_message: Failed to fetch message ${messageId}: ${err.message}`)
        return null
      })
      ...
      await message.unpin()
      logger.info(`unpin_message: Unpinned message ${messageId} in channel ${targetChannel.id}`)
      ...
      logger.error(`unpin_message failed: ${err.message}`)
      return { success: false, error: `Failed to unpin message: ${err.message}` }
    }
  }
}
