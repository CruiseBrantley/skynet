...
class TriggerEngine {
  ...
  async _dispatchTriggerAction (trigger, context, client) {
        ...
        try {
          ...
          if (owner) {
            const embed = new EmbedBuilder()
              .setTitle(`🚨 Watchdog Trigger Alert: ${trigger.description}`)
              .setColor(0xFF4500) // OrangeRed
              .setDescription('A reactive watchdog trigger condition was satisfied.')
              .addFields(
                { name: 'Condition', value: `\`${trigger.conditionType}\` (Threshold: ${trigger.threshold})`, inline: true },
                { name: 'Details', value: `\`\`\`json\n${JSON.stringify(context, null, 2)}\n\`\`\`` }
              )
              .setFooter({ text: `Trigger ID: ${trigger.id} • Cooldown: ${trigger.cooldownMinutes}m` })
              .setTimestamp()
    ...
    if (trigger.actionType === 'auto_heal') {
      const selfHealing = this.selfHealing || require('./chat/SelfHealingEngine')
